...
class CheckTicket:
    ...
    def process_item(self, item, spider):  # noqa: C901 #TODO: тут обработка билета и поиск по кондишенам
        if spider.mode == 'parse' and isinstance(item, Ticket):
    ...
    def close_spider(self, spider):  # noqa: C901
        if spider.mode == 'parse' and self.all_tickets:
...
class InitParamsAndCheckActuality:
    ...
    def open_spider(self, spider):
        ...
        if event := self.get_event_from_db(spider.id_event):
            ...
            if spider.mode == 'buy' and spider.event.skip_check: # сюда не заходил, убрать?
            ...
            if spider.event.count != 0 and conditions:
                ...
                logger.info('исчерпаны лимиты по count')
    ...
    #                 # send_sms_message(str(user), message, 'telegram')
    #         except Exception:  # pylint: disable=W0703
    ...
    @staticmethod
    def get_event_from_db(id_event):
        if client := get_mongo_bots_client():
            try:
                bots_db = client[settings.BOTS_DB]
...
class BotParseMode:
    ...
    def init_tickets_buy_tasks(self, spider):  #TODO: тут начинается закупка билетов СОРТИРУЙ
        if not self.tickets:
            return
...
class BotBuyMode:
    ...
    def open_spider(self, spider):
        ...
        if spider.mode == 'buy' and tickets:
            ...
            if client := get_mongo_bots_client():
                try:
                    ...
                    users = (spider.main_event['bot_params']['users']
                             + get_bot_admins())
                    # for user in users:
                        # send_sms_message(str(user), self.message, 'telegram')
                except Exception:  # pylint: disable=W0703
